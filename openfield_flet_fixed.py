...
class OpenFieldApp:
    ...
    def create_ui(self):
        ...
        self.corner_button = ft.Container(
            content=ft.Text("Canto", color=ft.colors.WHITE, size=16, weight=ft.FontWeight.BOLD),
            bgcolor=ft.colors.RED,
            padding=20,
            border_radius=8,
        ...
        self.lateral_button = ft.Container(
            content=ft.Text("Lateral", color=ft.colors.BLACK, size=16, weight=ft.FontWeight.BOLD),
        ...
        self.center_button = ft.Container(
            content=ft.Text("Centro", color=ft.colors.WHITE, size=16, weight=ft.FontWeight.BOLD),
            bgcolor=ft.colors.GREEN,
            padding=20,
            border_radius=8,
            alignment=ft.alignment.center,
    ...
    def start_test(self, e):
        ...
        if not self.animal_id_field.value.strip():
            self.show_snack_bar("Por favor, insira o ID do Animal.", ft.colors.RED)
            return
        ...
        try:
            ...
            self.show_snack_bar("Por favor, insira uma duração válida (número inteiro positivo).", ft.colors.RED)
            return
    ...
    def stop_test(self, e=None, manual_stop=True):
        ...
        if manual_stop:
            self.show_snack_bar(f"Teste para {self.animal_id} finalizado!", ft.colors.GREEN)
    ...
    def press_area_button(self, area):
        ...
        if area == "corner":
            ...
            self.highlight_button(self.corner_button, True, ft.colors.RED)
        elif area == "lateral":
            self.lateral_button_pressed = True
            self.lateral_press_time = current_time
            self.highlight_button(self.lateral_button, True, ft.colors.LIGHT_BLUE)
        elif area == "center":
            self.center_button_pressed = True
            self.center_press_time = current_time
            self.highlight_button(self.center_button, True, ft.colors.GREEN)
    ...
    def generate_report(self, e):
        if not self.start_time:
            self.show_snack_bar("Inicie um teste primeiro para gerar o relatório.", ft.colors.ORANGE)
            return
    ...
    def export_report(self, e):
        if not self.test_data:
            self.show_snack_bar("Nenhum relatório foi gerado para exportar.", ft.colors.ORANGE)
            return
        ...
        def save_file(file_path):
            try:
                ...
                self.show_snack_bar(f"Relatório exportado com sucesso!", ft.colors.GREEN)
            except Exception as ex:
                self.show_snack_bar(f"Erro ao exportar: {ex}", ft.colors.RED)
